...
class User(db.Model):
    ...
    def like_message(self, message):
        """Like a message."""
        if not self.is_liking(message):
            like = Likes(user_id=self.id, message_id=message.id)
            db.session.add(like)

    def unlike_message(self, message):
        """Unlike a message."""
        Likes.query.filter_by(user_id=self.id, message_id=message.id).delete()

    def is_liking(self, message):
        """Check if user likes the message."""
        return Likes.query.filter(
            Likes.user_id == self.id,
            Likes.message_id == message.id
        ).count() > 0
...
class Message(db.Model):
    ...
    def is_liked_by(self, user):
        """Check if this message is liked by the given user."""
        return bool(Likes.query.filter_by(user_id=user.id, message_id=self.id).first())
